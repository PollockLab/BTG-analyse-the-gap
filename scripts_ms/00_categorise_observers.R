...
100*table(user.obs$user_category)/nrow(user.obs)

# save
saveRDS(user.obs, "outputs/users/observer_categories.rds")


# how much did they contribute to BTG?

# first, all iNaturalist users
user.obs |>
  group_by(user_category) |>
  summarise("n_obs_total" = sum(n_obs, na.rm = T),
            "prop" = sum(n_obs, na.rm = T)/sum(user.obs$n_obs))
