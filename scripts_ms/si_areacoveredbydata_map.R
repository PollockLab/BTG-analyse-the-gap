...
base1k = project(base1k, crs(canada))

# load gbif raster
dat = terra::rast("outputs/summaries/map_gbif_inaturalistcontribution.tif")
plot(dat)
dat = resample(dat, base100k, method = "bilinear")

# smooth for plotting
dat_1k = resample(dat, base1k, method = "bilinear")
dat_smooth <- focal(dat_1k, w = 3, fun = mean, na.rm = T)
plot(dat_smooth)
...
# project
dat = project(dat, crs = "EPSG:3347")

# map
ggplot() +
  geom_sf(data = canada, fill = "grey95", col = "grey95") +
  geom_spatraster(data = dat_smooth) +
