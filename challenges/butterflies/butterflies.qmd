---
title: "Challenge: More than a Monarch"
author: Leeya Nagpal
...
df <- read.csv("C:/Users/dogpa/SpeciesList/relevantSpecies/completeList.csv")
butterflyList <- df[grepl("Lepidoptera", df$order, ignore.case = TRUE),]
butterflySpecies <- data.frame(butterflyList$species)
butterflySpecies <- data.frame(butterflySpecies[butterflySpecies$Species != "Danaus plexippus", ])
colnames(butterflySpecies) <- c("scientific_name")
```
...
```{r Analysis}
btgButterflies <- subset(butterflySpecies,butterflySpecies$Species %in% new$scientific_name)
withObs <- left_join(btgButterflies, new)
